...
$Event(90005432, Restart, function(chrEntityId, eventFlagId) {
    if (!EventFlag(eventFlagId)) {
        ...
        WaitFor(EventFlag(eventFlagId) || EntityInRadiusOfEntity(chrEntityId, 10000, 200, 1));
        if (!CharacterHasSpEffect(chrEntityId, 19627)) {
...
$Event(90005636, Restart, function(eventFlagId, chrEntityId, entityId, spEffectId, entityId2, areaEntityId, eventFlagId2, patrolInformationEntityId, value) {
    if (EventFlag(eventFlagId)) {
        DisableCharacter(chrEntityId);
        DisableCharacterCollision(chrEntityId);
        EndEvent();
...
L1:
    ...
    if (EventFlag(eventFlagId2)) {
        ...
        DisableCharacterAI(chrEntityId);
        if (flagArea.Passed) {
            WarpCharacterAndCopyFloor(chrEntityId, TargetEntityType.Area, entityId2, -1, 10000);
            WaitFixedTimeSeconds(0.1);
        }
L2:
...
$Event(90005764, Restart, function(chrEntityId, eventFlagId, eventFlagId2, eventFlagId3, eventFlagId4, eventFlagId5, eventFlagId6, animationId, range, entityId, assetEntityId) {
    DisableNetworkSync();
    WaitFixedTimeFrames(1);
    if (PlayerIsInOwnWorld()) {
        if (EventFlag(eventFlagId)) {
...
$Event(90005780, Default, function(eventFlagId, eventFlagId2, eventFlagId3, npcEntityId, signType, areaEntityId, eventFlagId4, shouldMultiplayerRestrictionsApply, value) {
    if (PlayerIsInOwnWorld()) {
    ...
    PlaceNPCSummonSign(signType, npcEntityId, areaEntityId, eventFlagId2, eventFlagId3, shouldMultiplayerRestrictionsApply);
    EndIf(Signed(0) == value);
});
...
$Event(90005790, Default, function(eventFlagId, eventFlagId2, eventFlagId3, eventFlagId4, npcEntityId, signType, areaEntityId, areaEntityId2, timeSeconds, eventFlagId5, shouldMultiplayerRestrictionsApply, value) {
    DisableNetworkSync();
    EndIf(!PlayerIsInOwnWorld());
    if (0 != eventFlagId) {
        EndIf(EventFlag(eventFlagId));
    ...
    WaitFixedTimeSeconds(timeSeconds);
    PlaceNPCSummonSign(signType, npcEntityId, areaEntityId, eventFlagId3, eventFlagId4, shouldMultiplayerRestrictionsApply);
    WaitFixedTimeSeconds(1);
    RestartEvent();
    EndIf(Signed(0) == value);
});
...
$Event(90005798, Restart, function(eventFlagId, chrEntityId, bannerType, areaEntityId, eventFlagId2) {
    DisableNetworkSync();
    EndIf(PlayerIsInOwnWorld());
    EndIf(EventFlag(eventFlagId));
    WaitFor(CharacterRatioDead(chrEntityId));
...
$Event(90005799, Restart, function(eventFlagId, chrEntityId, bannerType, areaEntityId, eventFlagId2, chrEntityId2) {
    DisableNetworkSync();
    EndIf(PlayerIsInOwnWorld());
    EndIf(EventFlag(eventFlagId));
    DisableCharacterCollision(chrEntityId2);
...
//action button for commence bullet
$Event(46990051, Restart, function(triggerAsset, teamSource, bulletProducer, isDuel, bannerDelay) {
    CreateAssetfollowingSFX(triggerAsset, 200, 806870);
    WaitFor(ActionButtonInArea(90010, triggerAsset));
    RotateCharacter(10000, triggerAsset, -1, true);
    ForceAnimationPlayback(10000, 60490, false, false, false);
    WaitFixedTimeSeconds(6);
    ShootBullet(teamSource, bulletProducer, -1, 46005000, 0, 0, 0);//start bullet
    WaitFixedTimeSeconds(bannerDelay);//2.75
    DisplayBanner(TextBannerType.Commence);
    if (isDuel == 1) {
        RestartEvent();
    }
    WaitFixedTimeSeconds(270);
    DisplayBlinkingMessage(470000);//30s remain
    WaitFixedTimeSeconds(30);
    ShootBullet(teamSource, bulletProducer, -1, 46005002, 0, 0, 0);//end bullet (all heal)
    WaitFixedTimeSeconds(3);//3 second delay as the bullet take a moment to appear
    DisplayBlinkingMessage(88040);
    RestartEvent();
});

//clean and label all this up
//------------V-------------


$Event(46990050, Restart, function(spEffectId, teamSource, bulletProducer) {
    WaitFor(CharacterHasSpEffect(10000, spEffectId));
    ShootBullet(teamSource, bulletProducer, -1, 46005000, 0, 0, 0);
    ClearSpEffect(10000, spEffectId);
    RestartEvent();
});

$Event(46990061, Restart, function(assetEntityId, chrEntityId, entityId) {
    CreateAssetfollowingSFX(assetEntityId, 200, 806870);
    WaitFor(ActionButtonInArea(90010, assetEntityId));
    RotateCharacter(10000, assetEntityId, -1, true);
    ForceAnimationPlayback(10000, 60490, false, false, false);
    WaitFixedTimeSeconds(6);
    ShootBullet(chrEntityId, entityId, -1, 46005000, 0, -1, 0);
    WaitFixedTimeSeconds(3);
    WaitFixedTimeSeconds(270);
    DisplayBlinkingMessageWithPriority(470000, 0, true);
    WaitFixedTimeSeconds(27);
    ShootBullet(chrEntityId, entityId, -1, 46005002, 0, 0, 0);
    WaitFixedTimeSeconds(3);
    RestartEvent();
});

$Event(46990060, Restart, function(spEffectId, chrEntityId, entityId) {
    WaitFor(CharacterHasSpEffect(10000, spEffectId));
    ShootBullet(chrEntityId, entityId, -1, 46005000, 0, -1, 0);
    WaitFixedTimeSeconds(3);
    WaitFixedTimeSeconds(270);
...
//waygate teleporters
$Event(46990062, Default, function(waygate, actionButtonId, destinationID, textId, option1, option2, option3) {
    DisableNetworkSync();
    CreateAssetfollowingSFX(waygate, 200, 806870);//waygate vfx
    SetEventFlagID(option1, OFF);
    SetEventFlagID(option2, OFF);
    WaitFor(ActionButtonInArea(actionButtonId, waygate) && !EventFlag(10));//action button trigger
    ForceAnimationPlayback(10000, 60510, false, true, false);//start anim
    //if not option 1 "OK" cancel
    if (HasDamageType(10000,0,DamageType.Unspecified)){
        RestartEvent();
    }
    DisplayGenericDialogAndSetEventFlags(textId, PromptType.OKCANCEL, NumberofOptions.TwoButtons, waygate, 3, option1, option2, option3);
    if (!EventFlag(option1)){
        SetEventFlagID(option3, ON);
        RestartEvent();
    }
    ForceAnimationPlayback(10000, 60505, false, false, false);//fade out anim
    WaitFixedTimeSeconds(2);
    WarpCharacterAndCopyFloorWithFadeout(10000, TargetEntityType.Area, destinationID, -1, 10000, false, true);
    WaitFixedTimeSeconds(1);
    ForceAnimationPlayback(10000, 60502, false, false, false);//fade in anim
    WaitFixedTimeSeconds(1);
...
//kill planes
$Event(46990063, Default, function(killPlane) {
    DisableNetworkSync();
    RestartIf(!InArea(10000, killPlane));
    WaitFor(InArea(10000, killPlane));
...
//rot planes
$Event(46990064, Default, function(rotPlane) {
    DisableNetworkSync();
    RestartIf(!InArea(10000, rotPlane));
    WaitFor(InArea(10000, rotPlane) && !CharacterHasSpEffect(10000, 20500671));
    SetSpEffect(10000, 20500671);
    RestartEvent();
});

//fast warp to arena
//w1asset = starting plate, w1region = cylinder warp region, w1 destination = sphere destination region
$Event(46990065, Restart, function (w1asset, w1region, w1destination, w2asset, w2region, w2destination) {
    DisableNetworkSync();
    SetEventFlagID(1, OFF);
    SetEventFlagID(2, OFF);
    WaitFor(ActionButtonInArea(90014, w1asset) || ActionButtonInArea(90014, w2asset));
    if (InArea(10000, w1region)) {
        SetEventFlagID(1, ON);
    }
    if (InArea(10000, w2region)) {
        SetEventFlagID(2, ON);
    }
    ForceAnimationPlayback(10000, 60505, false, false, false);
    WaitFixedTimeSeconds(2);
    if (EventFlag(1)) { WarpCharacterAndCopyFloor(10000, TargetEntityType.Area, w1destination, -1, 10000) }
    if (EventFlag(2)) { WarpCharacterAndCopyFloor(10000, TargetEntityType.Area, w2destination, -1, 10000) }
...
//fast warp red effect vfx
$Event(46990066, Restart, function (warpAssetID) {
    DisableNetworkSync();
    DeleteAssetfollowingSFX(warpAssetID, true);
    WaitFixedTimeSeconds(0.75);
    CreateAssetfollowingSFX(warpAssetID, 100, 4821);
    RestartEvent();
});

//fast warp blue effect vfx
$Event(46990067, Restart, function (warpAssetID) {
    DisableNetworkSync();
    DeleteAssetfollowingSFX(warpAssetID, true);
    WaitFixedTimeSeconds(0.75);
    CreateAssetfollowingSFX(warpAssetID, 100, 4811);
    RestartEvent();
});

//fast warp gold effect vfx
$Event(46990068, Restart, function (warpAssetID) {
    DisableNetworkSync();
    DeleteAssetfollowingSFX(warpAssetID, true);
    CreateAssetfollowingSFX(warpAssetID, 100, 4731);
    WaitFixedTimeSeconds(1);
    RestartEvent();
});

//fast warp red effect vfx w/ area
$Event(46990069, Restart, function (warpAssetID, areaID) {
    DisableNetworkSync();
...
//fast warp blue effect vfx w/ area
$Event(46990070, Restart, function (warpAssetID, areaID) {
    DisableNetworkSync();
    if(InArea(10000, areaID)){
    CreateAssetfollowingSFX(warpAssetID, 100, 4811);
    WaitFixedTimeSeconds(0.75);
...
//fast warp gold effect vfx w/ area
$Event(46990071, Restart, function (warpAssetID, areaID) {
    DisableNetworkSync();
    if(InArea(10000, areaID)){
    CreateAssetfollowingSFX(warpAssetID, 100, 4731);
    WaitFixedTimeSeconds(1);
...
//teleport to arenas
$Event(46990072, Restart, function(eventFlagId2, eventFlagId3, objectId, actionButtonId, messageId, areaId, blockId, regionId, indexId){
    DisableNetworkSync();
    EndIf(!PlayerIsInOwnWorld());
    SetEventFlagID(eventFlagId2, OFF);
    SetEventFlagID(eventFlagId3, OFF);
    WaitFor(ActionButtonInArea(actionButtonId, objectId));
    EndIf(HasMultiplayerState(MultiplayerState.Multiplayer));
    DisplayGenericDialogAndSetEventFlags(messageId, PromptType.OKCANCEL, NumberofOptions.TwoButtons, objectId, 3, eventFlagId2, eventFlagId3, eventFlagId3);
    if (!EventFlag(eventFlagId2)) {
        SetEventFlagID(eventFlagId3, ON);
        WaitFixedTimeSeconds(1);
        RestartEvent();
    }
    WarpPlayer(areaId, blockId, regionId, indexId, objectId, -11100);
});
